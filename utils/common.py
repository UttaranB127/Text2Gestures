...
def generate_simple_trajectories(scene_corners, skeleton_heights, character_heights,
                              num_characters_per_side, traj_history,
                              time_steps=50., radius=8.,
                              time_horizon=1.5, time_horizon_obstacle=2.,
                              show_plot=False):
    num_corners = len(scene_corners)
    num_characters = len(character_heights)
    num_skeletons = len(skeleton_heights)
    assert len(num_characters_per_side) == num_corners
    assert torch.sum(num_characters_per_side) == num_skeletons
    if num_skeletons > num_characters:
        character_heights = character_heights.repeat(int(np.ceil(num_skeletons / num_characters)))
    character_heights = character_heights[:num_skeletons]
    scale = torch.mean(skeleton_heights / character_heights)
    scaled_scene_corners = scene_corners.clone()
    for sidx in range(1, len(scaled_scene_corners)):
        scaled_scene_corners[sidx] = scaled_scene_corners[sidx - 1] +\
                                     scale * (scene_corners[sidx] - scene_corners[sidx - 1])
    start_pos = torch.zeros((num_skeletons, traj_history.shape[-1])).cuda().float()
    counter = 0
    for sidx in range(len(scaled_scene_corners)):
        start_pos[counter:counter + num_characters_per_side[sidx]] =\
            torch.from_numpy(np.linspace(scaled_scene_corners[sidx].cpu().numpy(),
                                         scaled_scene_corners[(sidx + 1) % num_corners].cpu().numpy(),
                                         int(num_characters_per_side[sidx].cpu().numpy()) + 2))[1:-1]
        counter += num_characters_per_side[sidx]
    end_permutations = np.roll(range(num_skeletons), int(num_skeletons / 2))
    end_pos = start_pos[end_permutations]
    preferred_velocities = (end_pos - start_pos)
    del_t = 1. / time_steps
    neighbor_dist = radius
    max_neighbors = num_skeletons
    max_speed = torch.max(torch.norm(preferred_velocities, dim=-1))
    sim = rvo2.PyRVOSimulator(del_t, neighbor_dist, max_neighbors,
                              time_horizon, time_horizon_obstacle, radius, max_speed)
    for a in range(num_skeletons):
        sim.addAgent((start_pos[a, 0], start_pos[a, 1]))
        sim.setAgentPrefVelocity(a, (preferred_velocities[a, 0], preferred_velocities[a, 1]))
    traj_markers = torch.zeros((num_skeletons, rvo_steps, traj_history.shape[-1])).cuda().float()
    traj_offsets = start_pos - traj_history[:, -1]
    return traj_markers, traj_offsets.unsqueeze(1).repeat(1, traj_history.shape[1], 1), scale


def generate_rvo_trajectories(scene_corners, skeleton_heights, character_heights,
                              num_characters_per_side, traj_history,
                              time_steps=50., radius=8.,
                              time_horizon=1.5, time_horizon_obstacle=2.,
                              show_plot=False):
    ...
    for a in range(num_skeletons):
        sim.addAgent((start_pos[a, 0], start_pos[a, 1]))
        sim.setAgentPrefVelocity(a, (preferred_velocities[a, 0], preferred_velocities[a, 1]))
    rvo_steps = int(time_steps)
    traj_markers = torch.zeros((num_skeletons, rvo_steps, traj_history.shape[-1])).cuda().float()
    for t in range(rvo_steps):
        sim.doStep()
        for a in range(num_skeletons):
            position = sim.getAgentPosition(a)
            traj_markers[a, t, 0] = position[0]
            traj_markers[a, t, 1] = position[1]
    plt.plot(scaled_scene_corners[np.arange(num_corners + 1) % num_corners, 0].cpu().numpy(),
             scaled_scene_corners[np.arange(num_corners + 1) % num_corners, 1].cpu().numpy())
    if show_plot:
        traj_np = traj_markers.cpu().numpy()
        for a in range(num_skeletons):
            plt.plot(traj_np[a, :, 0], traj_np[a, :, 1])
        plt.show()
    traj_offsets = start_pos - traj_history[:, -1]
    return traj_markers, traj_offsets.unsqueeze(1).repeat(1, traj_history.shape[1], 1), scale
